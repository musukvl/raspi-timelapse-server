...
class ServerBuilder {
    ...
    build() {
        ...
        co(function*() {
            ...
            self.configureSockets(appBuilder.app);
    ...
    configureSockets(app) {
        var self = this;
        self._io = SocketIO(self._server);
        self._streamService = new StreamService(self._appEnv, app, self._io);

        self._io.on('connection', function(socket) {

            self._sockets[socket.id] = socket;
            self._appEnv.logger().info("Total clients connected : ", Object.keys(self._sockets).length);

            self._streamService.onSocketConnect(socket);

            socket.on('disconnect', function() {
                delete self._sockets[socket.id];
                self._appEnv.logger().info('socket disconnect');

                // no more sockets, kill the stream
                if (Object.keys(self._sockets).length == 0) {
                    self._streamService.onLastSocketDisconnected();
                }
